...
//     let result = []
//     let search = searchFor[0].toUpperCase() + searchFor.slice(1)
//     for (const eachItem in items) {
//         if (items[eachItem][searchBy].startsWith(search)) {
//             result.push(items[eachItem])
//         }
//     }
//     return result
// }
...
function filterByKey(items, searchFor, searchBy) {
    let result = []
    for (const eachItem in items) {
        let newEachItem = items[eachItem][searchBy].toUpperCase()
        let newSearchFor = searchFor.toUpperCase()
        if (newEachItem.startsWith(newSearchFor)) {
            result.push(items[eachItem])
        }
    }
    return result
